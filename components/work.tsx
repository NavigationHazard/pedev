...
export const WorkRight: React.FC<Props> = ( { children, progress}) => {
    ...
    return(
         ...
         style={{transform: `translateY(${translateY})px` }}>
             <div className="w-full max-w-md lg:pt-10 pt-0 px-10 md:px-0"> {children}</div>
            </div>
    )
}
interface LinkProps {
...
export const WorkLink: React.FC<LinkProps> = ( {href , children}) => (
    <Link href={href}><a target="_blank"  className="underline underline-offset-8 decoration-3"> {children}</a></Link>
)
